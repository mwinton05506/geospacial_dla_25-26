...
class EDRPass(object):
    ...
    ['observer_ids'] : np.ndarrray (n_obs x 1)
        Satellite number associated with observations 
    """
    def __init__(self, ssusi_file, dmsp, hemisphere, radiance_type = 'LBHL', noise_removal = False, spatial_bin = False, minlat = 50):
        """
        Parameters
        ----------
        ssusi_file : str
    ...
    def _read_EDR_file(self, file_name):
        ...
        with h5py.File(file_name,'r') as h5f:
            ...
            disk['year'] = h5f['YEAR']*np.ones(1)[0]#*np.ones_like(disk['mlat']).flatten()
            disk['doy'] = h5f['DOY']*np.ones(1)[0]#*np.ones_like(disk['mlat']).flatten()
            ...
            if self.hemisphere == 'N':
                disk['radiance_all_colors'] = h5f['DISK_RADIANCEDATA_INTENSITY_NORTH'][:] / 1000.
                disk['time'] = h5f['UT_N'][:].flatten() * 3600.
                # disk['mlon'] = h5f['LONGITUDE_GEOMAGNETIC_NORTH_GRID_MAP'][:]
            else:
                disk['radiance_all_colors'] = h5f['DISK_RADIANCEDATA_INTENSITY_SOUTH'][:] / 1000.
                disk['time'] = h5f['UT_S'][:].flatten() * 3600.
        ...
        for k in range(len(dt)):
            dt[k,0] = datetime.datetime(int(disk['year']),1,1,0,0,0)+ \
                datetime.timedelta(days=int(disk['doy']-1.))+ \
    ...
    def ssusi_spatial_binning(self,disk_int):
        ...
        lats_in_pass, lts_in_pass, Y_in_pass = disk_int['mlat'], disk_int['mlt'], disk_int['Y']
        ...
        disk_binned['mlat'], disk_binned['mlon'], disk_binned['Y'] = binlats, binlons, binstats
    ...
    def plot_obs(self, ax, ptsize=1, startdt = None,enddt = None,hemisphere = None):
        ...
        r,theta = latlt2polar(lats.flatten(),lons.flatten()/180*12,'N') # change N to self.hemisphere ??

        # sc = ax.scatter(theta,r,c = obs, norm=clrs.LogNorm(vmin= 0.09,vmax=40), **kwargs)
        # sc = ax.scatter(theta,r,c = obs, **kwargs)
        sc = ax.scatter(theta,r,c = obs, norm=clrs.LogNorm(vmin= 0.09,vmax=40), s=ptsize, cmap='turbo')
